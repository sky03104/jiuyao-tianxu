...
| 空鏡／氛圍輔助素材 | MiniMax H3／Seedance（文字/圖片生成影片） | 僅限不需角色一致性的鏡頭，**輔助工具，非主線Pipeline** | 見2.2節 |

## 2.2 輔助工具：生成式影片模型（MiniMax H3／Veo 3／Seedance）

**（2026-09-16 咖哩提出候選，已查證後補入，僅作輔助定位）**

MiniMax H3、Veo 3、Seedance是「文字/圖片直接生成影片」的模型，跟本文件2.1節的
Blender+動作捕捉+Unity Cinematic是完全不同的技術路線——這類模型無法保證角色
跨鏡頭/跨集數的外觀一致性，**不能用於需要辨識為特定角色（如六位主要角色）的
劇情鏡頭**，牴觸本文件核心原則「固定角色+固定場景+Shot-by-Shot製作」。

**僅限用途**：
- 不需要角色一致性的空鏡／氛圍鏡頭（山脈雲霧、裂谷震動、城市遠景等過場畫面）。
- 短影音的快速測試/花絮素材。
- 前期分鏡/運鏡概念驗證。

**成本參考**（2026-09查證）：Seedance官方API約US$0.14/秒（480p~4K依解析度$0.067~
$0.78/秒不等，含音訊）；MiniMax H3約US$0.13/秒（2K）。一支60~90秒的空鏡輔助
素材成本大約US$8~13，屬於低成本試驗範圍。

**授權注意**：MiniMax H3開源權重的社群授權**明確排除美國/歐盟/英國/南韓**商用，
若考慮本地部署需先確認台灣是否受限；Seedance需透過ByteDance官方API或
Jimeng/Dreamina等付費方案才有明確商用授權，本地/開源版本授權需另查。Veo 3
（Google Cloud/Vertex AI服務）走雲端付費API，商用條款相對明確但屬於Google
生態系鎖定，非開源可自架方案。

**Tier分級**：Tier 3（輔助，非核心）——不影響本文件2.1節既定的Tier 1/2工具鏈，
僅在製作空鏡/氛圍/短影音花絮時視需求評估使用，不作為主線角色動畫的替代方案。
