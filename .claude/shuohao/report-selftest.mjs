...
ok(new Set(PANES.map((p) => p.flag)).size === PANES.length, '引數名不重複');
...
for (const p of PANES) {
  ...
  ok(p.needs.every((n) => PANES.some((x) => x.flag === n)), `${p.id} 依賴的都是已定義的引數`);
}
...

{
  ...
  ok(d.scripts[0].includes('addEventListener'), '取到的是指令碼正文');

  // 這一條是被真實 bug 逼出來的：各報告用 <script type="application/json"> 內嵌
  // 自己那份源 JSON 給匯出按鈕讀。當成指令碼摳出來執行，瀏覽器直接甩
  // `Unexpected token ':'`，而且五個面板一起廢。
  ok(d.body.includes('id="cast-data"'), '資料塊留在正文裡，不當指令碼處理');
  ok(!d.scripts.some((s) => s.includes('"source"')), '資料塊沒有被當成指令碼摳走');
  ok(!d.body.includes('addEventListener'), '真正的指令碼從正文裡剝掉了');
}

{
  // 帶 src 的外鏈指令碼不收（這個倉庫不產出，但別在這裡靜默吞掉）
  const d = splitDoc('<body><script src="x.js"><\/script><script>var a=1;<\/script></body>');
  eq(d.scripts.length, 1, '外鏈指令碼不當成內聯指令碼');
  ok(d.body.includes('src="x.js"'), '外鏈指令碼標籤留在正文裡');
}
...

{
  ...
  ok(js.startsWith('(function(){'), '包在 IIFE 裡——五份指令碼有重名的頂層 const');
  ok(js.includes('__root'), '拿到自己的面板根');
  ok(js.includes('new Proxy'), '用代理接管 document');
  ok(js.includes("document.querySelector('.expo')"), '原指令碼正文原樣保留，不做字串改寫');
  // 語法必須成立，否則整段指令碼在瀏覽器裡直接報錯
...

{
  ...
  ok(js.includes('catch'), '指令碼自己炸掉不連累別的面板');
}
...

{
  ...
  ok(pane.js.includes('__root'), '指令碼已加作用域');
  ok(!pane.html.includes('<script>'), '正文裡沒有留下裸指令碼');
}
...

{
  ...
  ok(html.startsWith('<!doctype html>'), '產出是一份完整檔案');
  ...
  ok(!/querySelectorAll\('\.(pane|nv|rail|main)[ '"]/.test(shellJs), '外殼指令碼查的也是帶字首的類名');

  // 每個面板的指令碼各成一塊，互不幹擾
  eq((html.match(/__root=window\.document\.getElementById/g) ?? []).length, 2, '每個面板一段作用域指令碼');
...

{
  ...
  ok(!/<script[^>]+\bsrc=/i.test(html), '沒有外鏈指令碼');
