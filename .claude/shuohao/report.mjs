 ...
 *   - **指令碼串味**：各報告的指令碼都是 `document.querySelector('.expo')` 這種全域性查詢。
 *     合成一頁後只會命中第一個——五個匯出按鈕會全廢。做法是給每份指令碼套一層
 ...
 *
 * `needs` 是這份報告渲染時要一併喂進去的上游——合併器手上有全部路徑，
 * 順手傳給各自的 render，使用者不用自己拼一長串引數。分鏡的 `--script`
 ...
 *
 * 已量過：五份報告跨檔案重複的 id 有 9 個（`#ep-1`…`#ep-6`、`#sec-gates`、
 * `#sec-scenes`、`#sec-rhythm`）。不改的話頁內錨點會跳到別的面板去。
 *
 * 一併改的引用點：`href="#…"`、`data-pane="…"`（大綱的圖表/表格切換靠它跟
 * `p.id` 比對）、`aria-controls`、`for`。**指令碼裡的 id 字串不用改**——
 ...
 * 給一份報告的指令碼套上作用域。
 *
 * 各報告的指令碼都寫成 `document.querySelector('.expo')` 這種全域性查詢——合成一頁
 * 之後只會命中第一個面板的那個，其餘四個匯出按鈕直接失效。這裡用一個 Proxy 把
 * `document` 換掉：查詢類方法限制在自己的 pane 內，`getElementById` 順手補上
 * id 字首（所以指令碼裡的 id 字串一個都不用改），其餘屬性與方法原樣透傳給真的
 * document（`createElement` 這些還要用）。
 *
 * 整段包在 IIFE 裡，順帶隔離各報告的頂層 `const`——五份指令碼裡有重名的。
 */
...
/**
 * 把一份完整報告檔案變成一個可以塞進外殼的面板。
 * 純函式，不碰檔案系統——自測直接喂 HTML 字串就能驗。
 */
 ...
 * 顯示，搜尋恢復全域性。預設分欄，因為五份加起來將近六十萬字元，一次全渲染很沉。
 */
