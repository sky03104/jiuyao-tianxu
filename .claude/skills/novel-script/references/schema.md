...
| `cliff` | string | 結尾懸唸的說明——最後一拍留什麼讓人點下一集。**必填** |
...
| 欄位 | 型別 | 說明 |
| --- | --- | --- |
| `sceneId` | string | `S01` 格式，對帳 art.json 的場景 |
| `lighting` | string | 該場用的光照狀態名，必須是 art.json 裡該場景登記過的狀態。可省略 |
| `characters` | string[] | 本場出場角色（`C01` 格式，對帳 outline.json）。空鏡給空陣列 |
| `props` | string[] | 本場用到的敘事道具（`P01` 格式，對帳 art.json）。可省略 |
...
| 欄位 | 說明 |
| --- | --- |
| `action` | 敘述體畫面描述，一拍一件事。**不許出現引號臺詞**（「」『』“”都不行）——臺詞混進動作就沒法計秒、沒法喂 TTS |
