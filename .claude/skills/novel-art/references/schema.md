...
美術設定集（場景 + 敘事道具）的載體。`dynasty` 是檔案級的——整部劇一個具體的真實朝代或年代（`明` / `北宋` / `民國`），場景、道具的形制與紋樣都按它寫；不許填「古代」「架空」這類含糊詞，理由見 `scene-pass.md` 第 1 條。**模型只填設計欄位**，場景與道具的出現集、承載爽點都由 `seed` 從 outline.json 搬運；大綱沒有 `props` 時道具表留空，由模型按 `prop-pass.md` 從原文提取。Markdown 和報告由 `render` 渲染。
...
| 欄位 | 必填 | 語言 | 說明 |
| --- | --- | --- | --- |
| `id` | 是 | — | `S01` 格式，全域性唯一。用 seed 時沿用 outline 的場景 id |
| `name` | 是 | 中文 | 場景名 |
| `primary` | 是 | — | 是不是主場景 |
...
| `image.sheet` | 是 | **英文** | 環境設定圖完整版面指令（見 `sheet.md`），必須整段包含 `scene-pass.md` 兩檔表面處理句之一 |
| `image.tags` | 是 | 英文 | 標籤陣列：題材、材質、光照、色板。不打畫風標，見 `scene-pass.md` |
...
| `carriedBy` | 否 | 中文 | 誰帶著它，自由文本 |
| `image.prompt` | 是 | **英文** | 白底主視角，**必須帶尺度短語、無人無手** |
| `image.negativePrompt` | 是 | **英文** | **必須禁人且禁手**（hands/fingers） |
| `image.sheet` | 是 | **英文** | 設定圖版面指令，**必須寫明 pure white background** + 同一套兩檔表面處理句之一 |
...
2. 光照狀態 ≥1 且都有英文提示詞
3. **空景**：反向提示詞必須禁人。環境和角色是兩層資產，混在一張圖裡一致性全毀
4. 出圖提示詞（主圖/反向/設定圖/光照）全部英文
...
7. 狀態 ≥1 且都有英文提示詞
8. 尺度參照寫進提示詞（scale 列舉對應的英文短語）
9. 反向提示詞禁手——拿著道具的手是最常見汙染
10. 設定圖純白背景可摳
...
node scripts/novel-art.mjs checkup art.json               # 只打印 10 道門 ✓/✗
```
