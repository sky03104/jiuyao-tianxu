...
段（segment）＝ 一次影片生成呼叫，≤ 15 秒，不跨場次
 ├─ 分鏡（cut）× 3–5 ＝ 段內剪下，每切 2–5 秒（硬門），各自認領劇本節拍
 ├─ 分鏡圖 ＝ 每切一張關鍵幀（<段號>/f1..fN.png）：主圖釘 0.00 秒，子圖釘各自切點
 └─ H3 提示詞 ＝ 一段一條，多圖對齊指令 + [Shot k] 切點時刻逐字對帳
```

- **兩人對話的正反打在一段裡一次生成**——全景、A 近景、B 近景各是一個 2–5 秒的分鏡，每格構圖由自己的分鏡圖控制，不靠文字賭
- **對齊指令是推匯出來的，不是寫出來的** — 多圖對齊句式（`Picture 2 aligns with the 3.00-second mark…`）和 `[Shot k] At 00:0X.XXX` 切點時刻全部由分鏡秒數推導，validate **逐字對帳**：改了秒數忘改提示詞，當場攔
...
- **分鏡圖是資產合成，不是憑空畫** — 每一格都帶一份掛圖清單：場景/角色/道具的設定圖。novel-art 和 novel-characters 的圖在這一步被消費。**本 skill 不出圖**，交付的是提示詞和這份清單
...
與倉庫裡另外四個 skill 同一主張：**checklist 交給模型自覺是靠不住的**。
...
| 景別詞 | 「特寫」這類中文景別詞必須出現在分鏡圖提示詞裡 |
| 運鏡詞表 | 運鏡直接用 H3 官方詞表（`Push In` / `Pan Left` / `Tracking Shot`…），且必須出現在**自己的 [Shot k] 段落**裡 |
| **H3 結構** | 首行對齊指令**由分鏡結構按語言推導、逐字對帳**；三欄位按序；每個 `[Shot k]` 的切點時刻等於前面分鏡秒數的累計 |
| **H3 臺詞逐字** | 認領的每句臺詞逐字出現在 `<d>` 塊裡，改一個標點都過不去 |
| **提示詞語言一致** | 正文語言與 `promptLang` 雙向對帳：設定中文寫成英文、設定英文混進中文，都攔 |
| 分鏡圖提示詞衛生 | 中文非空（直呼角色名——名字指向掛上去的設定圖） |
| 影片提示詞不含角色名 | H3 正文（中英文模式都查）和 Seedance 鏡頭正文不許出現角色名與別名——H3 與 Seedance 官方規範的要求。給 `--outline` / `--cast` 才查，不給**明說跳過** |
| **構圖量化欄位** | 每段有 `blocking`；每鏡焦距／機位／構圖／視線落點／焦點／穩定性齊全，穩定性在列舉裡 |
| **Seedance 鏡頭正文** | 每鏡 `shot` 中文非空，不寫秒數與時間碼、鏡頭編號、圖片引用、H3 標記和 `{}` `<>` `（）`——這些由程式按真實結構加 |
| 引用對帳 | 場次/人物/道具全部對帳劇本該場 |
...
| **失敗詳情長什麼樣** | 反覆出現卻沒有對應門的那類問題，只能靠人看這些自由文本發現 |

這是從 SkillOpt「skill 檔案是可訓練狀態」那套思路裡拿的一條：**檔案不是一次寫好的說明書，是要按反饋迭代的東西**——但迭代要有依據，而不是靠印象。日誌只累積證據，改不改、怎麼改仍然是人的判斷。
...
- 每格交付畫面提示詞 + 掛圖清單（場景/角色/道具設定圖），出圖在下游；H3 提示詞 + 整套分鏡圖直接下單給 MiniMax H3
...
node scripts/novel-storyboard.mjs validate sb.json --script script.json \
     --shots /path/to/cards                                              # 可選：開第 18 道配方門
...
323 項斷言，覆蓋節拍展開 / H3 骨架推導 / Seedance 拼裝 / 統計與批次 / 品質門逐項擊穿 / 配方卡庫解析與掛載 / seed / 渲染（含中英介面）/ H3 與 Seedance 匯出。不調模型、不花額度、1 秒跑完。改完指令碼先跑這個。
