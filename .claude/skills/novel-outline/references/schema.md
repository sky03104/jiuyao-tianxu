...
五件套的載體。**模型只管填這份 JSON**，Markdown 和 report.html 由 `render` 渲染出來，資產清單由指令碼彙總——五件套是四件模型寫 + 一件算出來的。
...
- `id` 格式 `B01`；`type` 自由文本（打臉/揭破/反轉…）；`weight` 只能 `major` / `minor`（預設算 minor）
...
node scripts/novel-outline.mjs checkup outline.json   # 只打印 14 道品質門 ✓/✗
```
